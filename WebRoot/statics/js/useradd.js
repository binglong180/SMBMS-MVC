...
$(function() {
	...
	if (errorinfo.val() == null || errorinfo.val() == "") {
		a_idPicPath.next().html("上传文件不能大于500KB,上传文件格式必须为：jpg，jpeg，png，pneg");
	} else {
...
$(function() {
	...
	/*
	 * $.ajax({ type:"GET",//请求类型 url:path+"/jsp/user.do",//请求的url
	 * data:{method:"getrolelist"},//请求参数 dataType:"json",//ajax接口（请求url）返回的数据类型
	 * success:function(data){//data：返回数据（json对象） if(data != null){
	 * userRole.html(""); var options = "<option value=\"0\">请选择</option>";
	 * for(var i = 0; i < data.length; i++){ //alert(data[i].id);
	 * //alert(data[i].roleName); options += "<option
	 * value=\""+data[i].id+"\">"+data[i].roleName+"</option>"; }
	 * userRole.html(options); } }, error:function(data){//当访问时候，404，500
	 * 等非200的错误状态码 validateTip(userRole.next(),{"color":"red"},imgNo+"
	 * 获取用户角色列表error",false); } });
	 */
	 ...
	 */
	/*
	 * userCode.bind("blur",function(){ //ajax后台验证--userCode是否已存在
	 * //user.do?method=ucexist&userCode=** $.ajax({ type:"GET",//请求类型
	 * url:path+"/jsp/user.do",//请求的url
	 * data:{method:"ucexist",userCode:userCode.val()},//请求参数
	 * dataType:"json",//ajax接口（请求url）返回的数据类型
	 * success:function(data){//data：返回数据（json对象） if(data.userCode ==
	 * "exist"){//账号已存在，错误提示 validateTip(userCode.next(),{"color":"red"},imgNo+ "
	 * 该用户账号已存在",false); }else{//账号可用，正确提示
	...
	addBtn.bind("click", function() {
		/*
		 * if(userCode.attr("validateStatus") != "true"){ userCode.blur(); }else
